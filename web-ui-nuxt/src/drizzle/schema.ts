...
export const userExerciseSentenceProgress = pgTable('user_exercise_sentence_progress', {
  userId: uuid('user_id').notNull(), // Ссылка на пользователя
  exerciseId: uuid('exercise_id').notNull().references(() => exercises.id), // Ссылка на упражнение
  sentenceId: uuid('sentence_id').notNull().references(() => sentences.id), // Ссылка на предложение
